...
from api.services.workflow.local_template import build_local_workflow_from_description
...
async def create_workflow_from_template(
    ...
    When MPS_API_URL is configured, uses the managed workflow generator.
    Otherwise builds a local starter workflow from the user's description
    (BYOK / self-hosted deployments without managed AI).
    """
    try:
        if MPS_API_URL:
            ...
            logger.info(
                "MPS_API_URL unset — creating local starter workflow for use_case={!r}",
                request.use_case,
            )
            workflow_data = build_local_workflow_from_description(
        ...
        capture_event(
            ...
            properties={
                "workflow_id": workflow.id,
                "workflow_name": workflow.name,
                "source": "template" if MPS_API_URL else "local_template",
                "call_type": request.call_type,
                "use_case": request.use_case,
                "organization_id": user.selected_organization_id,
            },
