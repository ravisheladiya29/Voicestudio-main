...
from api.branding import normalize_brand_text
...
def _require_managed_ai() -> None:
    if not MPS_API_URL or not MPS_API_URL.strip():
        raise HTTPException(
            status_code=503,
            detail=(
                "Managed AI is not configured. Set MPS_API_URL on the API "
                "(for example https://services.dograh.com), then recreate the API container."
            ),
        )


def _to_service_key_response(key: dict) -> ServiceKeyResponse:
    response = ServiceKeyResponse.model_validate(key)
    normalized_name = normalize_brand_text(response.name)
    if normalized_name != response.name:
        return response.model_copy(update={"name": normalized_name})
    return response
...
async def get_service_keys(
    ...
    try:
        ...
        return [_to_service_key_response(key) for key in service_keys]
