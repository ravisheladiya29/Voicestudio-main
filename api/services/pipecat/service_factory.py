...
def _require_mps_api_url() -> str:
    """Managed (dograh) providers need an MPS-compatible backend URL."""
    if not MPS_API_URL or not MPS_API_URL.strip():
        raise HTTPException(
            status_code=503,
            detail=(
                "Managed AI is not configured. Set MPS_API_URL on the API to an "
                "MPS-compatible backend (for example https://services.dograh.com), "
                "then recreate the API container."
            ),
        )
    return MPS_API_URL.strip()
...
def create_stt_service(
    ...
    if user_config.stt.provider == ServiceProviders.DEEPGRAM.value:
        ...
        mps_url = _require_mps_api_url()
        base_url = mps_url.replace("http://", "ws://").replace("https://", "wss://")
        language = getattr(user_config.stt, "language", None) or "multi"
