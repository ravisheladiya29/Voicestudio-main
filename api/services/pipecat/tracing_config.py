...
from api.branding import BRAND_NAME
...
# OTEL attribute used to route spans to per-org Langfuse projects.
# Keep this brand-specific key stable once shipped; renaming breaks routing
# for in-flight exporters until restart.
ORG_ID_SPAN_ATTRIBUTE = "zenvoice.org_id"
# Legacy upstream key — still read so older in-memory spans keep routing.
_LEGACY_ORG_ID_SPAN_ATTRIBUTE = "dograh.org_id"
...
class _OrgAttributeSpanProcessor(SpanProcessor):
    ...
    def on_start(self, span, parent_context=None):
        ...
        if org_id:
            span.set_attribute(ORG_ID_SPAN_ATTRIBUTE, str(org_id))
...
class _OrgRoutingExporter(SpanExporter):
    ...
    Spans with a ``zenvoice.org_id`` attribute whose org has registered
    ...
    def export(self, spans):
        ...
        for span in spans:
            ...
            org_id = attrs.get(ORG_ID_SPAN_ATTRIBUTE) or attrs.get(
                _LEGACY_ORG_ID_SPAN_ATTRIBUTE
            )
...
def ensure_tracing() -> bool:
    ...
    setup_tracing(
        service_name=f"{BRAND_NAME.lower()}-pipeline",
        exporter=_org_routing_exporter,
    )
...
def get_trace_url(trace_id: str, org_id=None) -> str | None:
    ...
    if not host:
        # Prefer the browser-facing URL for "View Trace" links; fall back to
        # LANGFUSE_HOST (used for OTEL export from inside Docker).
        host = LANGFUSE_PUBLIC_URL or LANGFUSE_HOST
    if not host:
        return None
