...
DOGRAH_PROVIDER_MODEL_CONFIG = provider_model_config(BRAND_NAME)
...
class DograhLLMService(BaseLLMConfiguration):
    ...
    model: str = Field(
        default="default",
        description=f"{BRAND_NAME}-hosted model tier.",
...
class GoogleTTSConfiguration(BaseTTSConfiguration):
    ...
    model: str = Field(
        default="chirp_3_hd",
        description=(
            f"Google Cloud low-latency TTS engine. {BRAND_NAME} maps this to Pipecat's "
...
class DograhTTSService(BaseTTSConfiguration):
    ...
    model: str = Field(
        default="default",
        description=f"{BRAND_NAME} TTS tier.",
...
class DograhSTTService(BaseSTTConfiguration):
    ...
    model: str = Field(
        default="default",
        description=f"{BRAND_NAME} STT tier.",
...
class DograhEmbeddingsConfiguration(BaseEmbeddingsConfiguration):
    ...
    model: str = Field(
        ...
        description=f"{BRAND_NAME}-managed embedding model.",
