...
ZENVOICE_MCP_INSTRUCTIONS = """\
You build and edit Zenvoice voice-AI workflows **interactively** with the user using TypeScript that uses the `@dograh/sdk` package. Workflows are stored as JSON; this server projects them to TypeScript for editing and parses them back on save.

## Planning and workflow creation

Every authoring sessiouns through three stages. Inject the right guidance at each by calling `get_voice_prompting_guide` before you write or revise prompts. You must go through plan phase to gather context from the builder before attempting to build the agent.
...
1. `search_docs` — use first for keyword or acronym lookup when the user is asking how Zenvoice works or how to configure something.
...
### Editing an existn ring workflow
