...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `zenvoice.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `zenvoice.org_id` — that attribute triggers the
