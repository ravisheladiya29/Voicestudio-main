...
if [[ $EUID -ne 0 ]]; then
    zenvoice_fail "setup_remote.sh must be run as root.\nRe-run with sudo:\n  sudo ./setup_remote.sh"
fi
...
if [[ -z "$SERVER_IP" ]]; then
    zenvoice_fail "IP address cannot be empty"
fi

if ! zenvoice_is_ipv4 "$SERVER_IP"; then
    zenvoice_fail "Invalid IP address format"
fi
...
if [[ "$CERT_MODE" == "auto" ]]; then
    if zenvoice_is_local_ipv4 "$SERVER_IP"; then
        ...
        zenvoice_warn "$SERVER_IP is a private IP — using a self-signed certificate."
        zenvoice_warn "For a trusted cert, deploy on a public IP or a domain you own"
        zenvoice_warn "(https://docs.zenvoice.example/deployment/custom-domain)."
    elif ! command -v docker >/dev/null 2>&1; then
        CERT_MODE="self-signed"
        zenvoice_warn "Docker not found — skipping automatic Let's Encrypt setup and using a self-signed cert."
    else
        CERT_MODE="sslip"
    fi
fi
...
case "$CERT_MODE" in
    ...
    sslip)
        if zenvoice_is_local_ipv4 "$SERVER_IP"; then
            zenvoice_fail "CERT_MODE=sslip needs a public IP; $SERVER_IP is private/reserved."
        fi
        command -v docker >/dev/null 2>&1 || zenvoice_fail "CERT_MODE=sslip needs Docker to serve the ACME challenge."
        ;;
    letsencrypt-dns|cloudflare-tunnel|external)
        zenvoice_fail "CERT_MODE=$CERT_MODE is reserved but not implemented yet. Use 'sslip' (public IP) or 'self-signed'."
        ;;
    *)
        zenvoice_fail "Unknown CERT_MODE '$CERT_MODE' (expected: auto, sslip, self-signed)."
        ;;
esac

if [[ "$CERT_MODE" == "sslip" ]]; then
    PUBLIC_HOST_VALUE="$(zenvoice_sslip_host_from_ip "$SERVER_IP" "$ACME_DOMAIN_SUFFIX")"
...
if [[ -z "${DEPLOY_MODE:-}" ]]; then
    if [[ -t 0 ]]; then
        ...
        echo "  1) prebuilt - pull official zenvoice images (recommended, fastest)"
        ...
        case "$mode_choice" in
            ...
            *) zenvoice_fail "invalid choice '$mode_choice'" ;;
        esac
    else
    ...
    if [[ "$REPO_SOURCE" == "clone" ]]; then
        if [[ -z "${FORK_REPO:-}" ]]; then
            if [[ -t 0 ]]; then
                ...
                read -p "[$DEPLOY_BUNDLE_REPO]: " FORK_REPO
                FORK_REPO="${FORK_REPO:-$DEPLOY_BUNDLE_REPO}"
            else
                FORK_REPO="$DEPLOY_BUNDLE_REPO"
            fi
        fi
...
[[ "$FASTAPI_WORKERS" =~ ^[1-9][0-9]*$ ]] || zenvoice_fail "FASTAPI_WORKERS must be a positive integer (got: $FASTAPI_WORKERS)"
...
if [[ "${ZENVOICE_FORCE_OVERWRITE:-}" != "1" && "${ZENVOICE_SKIP_DOWNLOAD:-}" != "1" ]]; then
    if [[ -f "$TARGET_DIR/.env" ]]; then
        ...
        echo ""
        echo -e "${YELLOW}Detected an existing Zenvoice install:${NC}"
        ...
        echo -e "  ${BLUE}https://docs.zenvoice.example/deployment/update${NC}"
        echo ""
        echo -e "${BLUE}To wipe state and reinstall from scratch, re-run with:${NC}"
        echo -e "  ${BLUE}ZENVOICE_FORCE_OVERWRITE=1 <same command>${NC}"
        echo ""
        exit 1
    fi
fi
...
if [[ "$DEPLOY_MODE" == "build" ]]; then
    if [[ "${ZENVOICE_SKIP_DOWNLOAD:-}" == "1" ]]; then
        echo -e "${BLUE}[1/$TOTAL] Using existing repo in current directory${NC}"
    elif [[ "${REPO_SOURCE:-}" == "clone" ]]; then
        if [[ -e "zenvoice" ]]; then
            zenvoice_fail "'zenvoice' directory already exists. Remove it or re-run with REPO_SOURCE=existing from inside it."
        fi
        echo -e "${BLUE}[1/$TOTAL] Cloning $FORK_REPO (branch: $BRANCH)...${NC}"
        git clone --branch "$BRANCH" --recurse-submodules "https://github.com/$FORK_REPO.git" zenvoice
        cd zenvoice
    ...
    if [[ "${ZENVOICE_SKIP_DOWNLOAD:-}" != "1" ]]; then
        mkdir -p zenvoice 2>/dev/null || true
        cd zenvoice

        echo -e "${BLUE}[1/$TOTAL] Downloading deployment bundle...${NC}"
        curl -fsSL -o docker-compose.yaml "https://raw.githubusercontent.com/$DEPLOY_BUNDLE_REPO/main/docker-compose.yaml"
        zenvoice_download_remote_support_bundle "$(pwd)" "main"
...
ZENVOICE_DEPLOY_PROJECT_DIR="$(pwd)"
...
MINIO_ROOT_USER="zenvoice$(openssl rand -hex 6)"
...
zenvoice_prepare_remote_install "$(pwd)"
...
services:
  api:
    ...
    image: zenvoice-local/zenvoice-api:local
    pull_policy: never
  ...
  ui:
    ...
    image: zenvoice-local/zenvoice-ui:local
    pull_policy: never
OVERRIDE_EOF
...
if [[ "$CERT_MODE" == "sslip" ]]; then
    echo ""
    echo -e "${BLUE}Starting Zenvoice and requesting a trusted certificate for ${PUBLIC_HOST_VALUE}...${NC}"
    ...
    if [[ "$nginx_ready" != "1" ]]; then
        ...
        zenvoice_warn "nginx did not become reachable on port 80 — skipping Let's Encrypt for now."
        zenvoice_warn "The stack is running with the bootstrap self-signed certificate."
    elif zenvoice_install_certbot && zenvoice_issue_letsencrypt_webroot "$(pwd)" "$PUBLIC_HOST_VALUE" "$LETSENCRYPT_EMAIL"; then
        docker compose --profile remote restart nginx >/dev/null 2>&1 || true
        zenvoice_install_cert_renewal_hook "$(pwd)" "$PUBLIC_HOST_VALUE"
        CERT_RESULT="sslip"
        zenvoice_success "✓ Trusted Let's Encrypt certificate installed; auto-renewal configured"
    else
        CERT_RESULT="self-signed"
        echo ""
        zenvoice_warn "Let's Encrypt issuance failed — the stack is running with the self-signed certificate."
        zenvoice_warn "Common causes and fixes:"
        zenvoice_warn "  - Port 80 not reachable from the internet: open it in your firewall/security group"
        zenvoice_warn "  - Rate limited on ${ACME_DOMAIN_SUFFIX}: re-run with ACME_DOMAIN_SUFFIX=nip.io"
        zenvoice_warn "  - Then retry: sudo certbot certonly --webroot -w \"$(pwd)/certs\" -d ${PUBLIC_HOST_VALUE}"
    fi
fi
...
echo "  - remote_up.sh"
echo "  - scripts/run_zenvoice_init.sh"
...
if [[ "$CERT_MODE" == "sslip" ]]; then
    if [[ "$CERT_RESULT" == "sslip" ]]; then
        echo -e "${GREEN}Zenvoice is running with a trusted certificate at:${NC}"
        echo ""
        ...
        echo -e "${YELLOW}Zenvoice is running (with a temporary self-signed certificate) at:${NC}"
        echo ""
        echo -e "  ${BLUE}https://$PUBLIC_HOST_VALUE${NC}"
        echo ""
    ...
    echo -e "${YELLOW}To start Zenvoice, run:${NC}"
    echo ""
    if [[ "$DEPLOY_MODE" != "build" || "${REPO_SOURCE:-}" != "existing" ]]; then
