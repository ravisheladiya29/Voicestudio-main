#!/usr/bin/env pwsh
# Start Zenvoice services in development mode (Windows)
...
$EnvFile    = if ($env:ZENVOICE_ENV_FILE) { $env:ZENVOICE_ENV_FILE } else { Join-Path $BaseDir 'api/.env' }
...
Write-Host "Starting Zenvoice Services (DEV MODE) in BASE_DIR: $BaseDir"
