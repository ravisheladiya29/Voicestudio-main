...
function New-MinioRootUser {
    return "zenvoice$((New-HexSecret).Substring(0, 12))"
}
...
function Sync-PostgresPassword {
    ...
    "ALTER USER postgres WITH PASSWORD :'zenvoice_password';" | docker compose exec -T postgres psql `
        -U postgres `
        -d postgres `
        -v 'ON_ERROR_STOP=1' `
        -v "zenvoice_password=$Password" > $null
    if ($LASTEXITCODE -ne 0) {
...
$answer = Read-Host 'Start Zenvoice now? [Y/n]'
if ($answer -match '^[Nn]') {
    Write-Host 'Zenvoice was not started.'
    exit 0
}
