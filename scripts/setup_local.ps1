...
function Download-BundleFileForRef([string]$Destination, [string]$RemotePath, [string]$Ref) {
    $bundleRepo = if ($env:DEPLOY_BUNDLE_REPO) { $env:DEPLOY_BUNDLE_REPO } else { 'dograh-hq/dograh' }
    $rawBase = "https://raw.githubusercontent.com/$bundleRepo/$Ref"
    $fallbackBase = "https://raw.githubusercontent.com/$bundleRepo/main"
...
function Download-InitScript([string]$Destination, [string]$Ref) {
    $bundleRepo = if ($env:DEPLOY_BUNDLE_REPO) { $env:DEPLOY_BUNDLE_REPO } else { 'dograh-hq/dograh' }
    $rawBase = "https://raw.githubusercontent.com/$bundleRepo/$Ref"
    $fallbackBase = "https://raw.githubusercontent.com/$bundleRepo/main"

    try {
        Download-File "$rawBase/scripts/run_zenvoice_init.sh" $Destination
        return
    } catch {
        if ($Ref -ne 'main') {
            try {
                Write-Warn "Warning: run_zenvoice_init.sh not found at '$Ref' - falling back to main"
                Download-File "$fallbackBase/scripts/run_zenvoice_init.sh" $Destination
                return
            } catch {
                # fall through to upstream init script name
            }
        }
    }

    Write-Warn 'Warning: run_zenvoice_init.sh not found - falling back to upstream run_dograh_init.sh'
    Download-BundleFileForRef $Destination 'scripts/run_dograh_init.sh' $Ref
}
...
function Download-InitSupportBundle([string]$ProjectDir, [string]$Ref) {
    ...
    Download-InitScript (Join-Path $ProjectDir 'scripts/run_zenvoice_init.sh') $Ref
...
Write-Info ''
Write-Info '╔══════════════════════════════════════════════════════════════╗'
Write-Info '║                    Zenvoice Local Setup                      ║'
...
$Registry = if ([string]::IsNullOrEmpty($env:REGISTRY)) { 'ghcr.io/zenvoice' } else { $env:REGISTRY }
...
if ($env:ZENVOICE_SKIP_DOWNLOAD -ne '1') {
    if ($UseCoturn) {
    ...
    $bundleRepo = if ($env:DEPLOY_BUNDLE_REPO) { $env:DEPLOY_BUNDLE_REPO } else { 'dograh-hq/dograh' }
    Download-File "https://raw.githubusercontent.com/$bundleRepo/main/docker-compose.yaml" (Join-Path $CurrentDir 'docker-compose.yaml')
    if ($UseCoturn) {
...
if ($UseCoturn) {
    Assert-PathExists 'scripts/run_zenvoice_init.sh' 'scripts/run_zenvoice_init.sh not found. Re-run setup_local.ps1 without ZENVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'scripts/lib/setup_common.sh' 'scripts/lib/setup_common.sh not found. Re-run setup_local.ps1 without ZENVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'deploy/templates/turnserver.remote.conf.template' 'deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.ps1 without ZENVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
}
...
$minioRootUser = "zenvoice$((New-HexSecret 6).Substring(0, 12))"
$minioRootPassword = New-HexSecret 32

$envLines = @(
    '# Container registry for Zenvoice images'
    "REGISTRY=$Registry"
    ''
