...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              Zenvoice Custom Domain Setup                    ║"
echo "║     Automated Let's Encrypt SSL certificate setup            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

if [[ $EUID -ne 0 ]]; then
    zenvoice_fail "This script must be run as root or with sudo"
fi

if [[ ! -d "zenvoice" ]]; then
    echo -e "${RED}Error: 'zenvoice' directory not found.${NC}"
    echo -e "${YELLOW}Please run this script from the directory containing your Zenvoice installation.${NC}"
    echo -e "${YELLOW}If you haven't set up Zenvoice yet, run the remote setup first:${NC}"
    echo -e "${BLUE}  curl -o setup_remote.sh https://raw.githubusercontent.com/$DEPLOY_BUNDLE_REPO/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && sudo ./setup_remote.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Enter your domain name (e.g., voice.yourcompany.com):${NC}"
read -p "> " DOMAIN_NAME
[[ -n "$DOMAIN_NAME" ]] || zenvoice_fail "Domain name cannot be empty"

if ! [[ "$DOMAIN_NAME" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
    zenvoice_fail "Invalid domain name format"
fi

echo -e "${YELLOW}Enter your email address for SSL certificate notifications:${NC}"
read -p "> " EMAIL_ADDRESS
[[ -n "$EMAIL_ADDRESS" ]] || zenvoice_fail "Email address cannot be empty (required by Let's Encrypt)"
...
if [[ -z "$SERVER_IP" ]]; then
    zenvoice_warn "Warning: Could not detect server's public IP"
...
zenvoice_install_certbot || zenvoice_fail "Could not install certbot. Please install it manually and re-run."
...
zenvoice_require_init_compose_layout "$(pwd)"

zenvoice_load_env_file .env
if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(zenvoice_infer_server_ip "$(pwd)" || true)"
fi
[[ -n "${SERVER_IP:-}" ]] || zenvoice_fail "Could not determine SERVER_IP from the existing install"

zenvoice_set_env_key .env SERVER_IP "$SERVER_IP"
zenvoice_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
zenvoice_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
zenvoice_delete_env_key .env BACKEND_URL
...
zenvoice_delete_env_key .env BACKEND_API_ENDPOINT
zenvoice_delete_env_key .env MINIO_PUBLIC_ENDPOINT
zenvoice_delete_env_key .env TURN_HOST
zenvoice_prepare_remote_install "$(pwd)"

# Bring the stack up (recreating it) so zenvoice-init re-renders nginx with the
...
[[ "$nginx_ready" == "1" ]] || zenvoice_fail "nginx did not come up on port 80; cannot run the ACME challenge."
...
if ! zenvoice_issue_letsencrypt_webroot "$(pwd)" "$DOMAIN_NAME" "$EMAIL_ADDRESS"; then
...
zenvoice_install_cert_renewal_hook "$(pwd)" "$DOMAIN_NAME"
...
echo -e "  Certificate: $ZENVOICE_PATH/certs/local.crt"
echo -e "  Private Key: $ZENVOICE_PATH/certs/local.key"
echo -e "  Auto-renewal: Enabled (certificates renew automatically)"
echo ""
echo -e "${YELLOW}Files modified:${NC}"
echo "  - zenvoice/.env (canonical public host/base URL updated)"
echo "  - zenvoice/certs/local.crt (SSL certificate)"
echo "  - zenvoice/certs/local.key (SSL private key)"
echo "  - /etc/letsencrypt/renewal-hooks/deploy/zenvoice-reload.sh (renewal hook)"
echo ""
