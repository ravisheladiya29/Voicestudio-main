...
REGISTRY="${REGISTRY:-ghcr.io/zenvoice}"
...
# Download compose file (skip when ZENVOICE_SKIP_DOWNLOAD=1 — e.g. local repo testing).
TOTAL_STEPS=2
...
if [[ "${ZENVOICE_SKIP_DOWNLOAD:-}" != "1" ]]; then
    ...
    curl -sS -o docker-compose.yaml "https://raw.githubusercontent.com/$DEPLOY_BUNDLE_REPO/main/docker-compose.yaml"
    if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
        zenvoice_download_init_support_bundle "$(pwd)" "main"
    fi
...
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    [[ -f scripts/run_zenvoice_init.sh ]] || zenvoice_fail "scripts/run_zenvoice_init.sh not found. Re-run setup_local.sh without ZENVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f scripts/lib/setup_common.sh ]] || zenvoice_fail "scripts/lib/setup_common.sh not found. Re-run setup_local.sh without ZENVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f deploy/templates/turnserver.remote.conf.template ]] || zenvoice_fail "deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.sh without ZENVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout."
fi
...
echo ""
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    echo -e "${YELLOW}To start Zenvoice with TURN, run:${NC}"
    echo ""
