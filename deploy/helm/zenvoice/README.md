...
Deploys Zenvoice on Kubernetes with decomposed backend workloads (web,
...
|------------------------------|-------------|-----------------|-------|
| `zenvoice-web`                 | 2 (HPA opt) | RollingUpdate   | Long-lived WS, graceful drain |
| `zenvoice-arq-worker`          | 1 (knob)    | RollingUpdate   | Stateless |
| `zenvoice-ari-manager`         | **1 fixed** | **Recreate**    | Telephony singleton |
| `zenvoice-campaign-orchestrator` | **1 fixed** | **Recreate**  | Campaign singleton (in-memory locks) |
| `zenvoice-ui`                  | 2           | RollingUpdate   | Next.js SSR |
| `zenvoice-coturn`              | 1           | Recreate        | LoadBalancer Service, port-pinned |
