...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.zenvoice.example). The route will set
  // the cookie for the *current* sub-domain only and then forward the user to the
  // final destination.
  // ---------------------------------------------------------------------------------

  // Determine the base URL that should handle the impersonation cookie. If we are on
  // superadmin.zenvoice.example we want to switch to app.zenvoice.example. For any other domain
