// Thin client for a SEPARATE, optional user_onboarding/lead-capture service
// (its own base URL). All endpoints are PUBLIC (no auth token); identity is
// the email carried in the body. Every call is BEST-EFFORT: failures are
// swallowed so a down/erroring service never blocks the user.

// Base URL of the user_onboarding service. Empty by default — this
// whitelabeled deployment has no lead-capture backend of its own, so
// submissions no-op rather than silently phoning home to a third party. Set
// NEXT_PUBLIC_ONBOARDING_API_URL to your own service to enable this feature.
