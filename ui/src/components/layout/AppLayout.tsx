...
function BackendStatusBanner() {
  ...
  // Prefer the browser-facing API URL in the banner. `backendUrl` is the
  // UI container's internal Docker hostname (http://api:8000), which is
  // confusing when shown to operators in the browser.
  const backendUrl =
    config.backendApiEndpoint ||
    (config.backendUrl && config.backendUrl !== "unknown" ? config.backendUrl : null) ||
    "the configured backend";
  const rawMessage = config.backendMessage || `Backend is not reachable at ${backendUrl}.`;
  const message = rawMessage.replaceAll("http://api:8000", backendUrl);
