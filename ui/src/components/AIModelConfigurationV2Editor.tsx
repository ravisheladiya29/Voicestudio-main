        ...
        <div className="mb-4 flex gap-3 rounded-md border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
            ...
            <div>
                ...
                <p className="mt-1 leading-6">
                    {BRAND_NAME} sends data required by the selected model service. This may include prompts,
...
export function AIModelConfigurationV2Editor({
    ...
    const managedAiEnabled = config?.managedAiEnabled ?? false;
    const defaultsForByok = useMemo(() => byokDefaults(defaults), [defaults]);
    // Managed mode is stored as "dograh" in the API schema (provider id).
    // Default to BYOK for new orgs; preferredMode() restores Managed when saved.
        ...
        <div className="space-y-6">
            ...
            <Tabs value={mode} onValueChange={(value) => setMode(value as ModelMode)} className="space-y-6">
                ...
                <TabsContent value="dograh" className="mt-0">
                    <p className="mb-4 text-sm text-muted-foreground">
                        Use {BRAND_NAME}-managed LLM, STT, and TTS. Create a{" "}
                        <a href="/api-keys" className="underline underline-offset-2">
                            Service Key
                        </a>{" "}
                        under Developers, paste it below, then choose voice and language.
                    </p>
                    {!managedAiEnabled && (
                        <div className="mb-4 flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/30">
                            <Info className="mt-0.5 h-4 w-4 flex-shrink-0 text-amber-600 dark:text-amber-400" />
                            <p className="text-sm text-amber-900 dark:text-amber-200">
                                Managed AI backend is not configured. The operator must set{" "}
                                <code className="text-xs">MPS_API_URL</code> on the API
                                (for example <code className="text-xs">https://services.dograh.com</code>)
                    ...
                    <Card>
                        <CardContent className="pt-6">
                            <div className="grid gap-4 sm:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="dograh-api-key">{BRAND_NAME} Service Key</Label>
