...
import { getProviderDisplayName } from "@/config/brand";
...
function getGlobalSummary(
    ...
    const providerLabel = getProviderDisplayName(provider, providerSchema?.title);
...
export function ServiceConfigurationForm({
    ...
    const renderServiceFields = (service: ServiceSegment) => {
        const currentProvider = serviceProviders[service];
        const providerSchema = schemas?.[service]?.[currentProvider];
        // Managed AI uses the Dedicated "Managed" tab (provider id "dograh").
        // Keep it out of BYOK provider lists to avoid duplicate paths.
        const availableProviders = schemas?.[service]
            ? Object.keys(schemas[service]).filter((provider) => provider !== "dograh")
            : [];
                            ...
                            <SelectContent>
                                {availableProviders.map((provider) => (
                                    <SelectItem key={provider} value={provider}>
                                        {getProviderDisplayName(provider, schemas?.[service]?.[provider]?.title)}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
