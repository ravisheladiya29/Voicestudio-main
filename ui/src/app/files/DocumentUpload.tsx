  ...
  const ossNotice = isOSS ? (
    <div className="flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-900/50 dark:bg-amber-950/30">
      ...
      <div className="text-xs text-amber-900 dark:text-amber-200">
        <p className="font-medium">Requires a document-processing backend</p>
        <p className="mt-1">
          Uploaded documents are sent to a Model Proxy Service (MPS) for parsing and
          chunking. This self-hosted build of {BRAND_NAME} does not include a managed
          MPS, so document processing is unavailable unless you configure your own
          MPS-compatible backend (MPS_API_URL).
        </p>
      </div>
    </div>
  ) : null;
